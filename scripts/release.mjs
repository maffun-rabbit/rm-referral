...
else if(mode==='deploy'){const {config}=requireLanguage(locale);await prepareRelease();const r=spawnSync('npx',['--no-install','wrangler','deploy','--config',config],{cwd:root,stdio:'inherit'});if(r.status!==0)process.exit(r.status??1);}
