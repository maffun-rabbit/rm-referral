...
async function validateSitemap(directory, host, siteFiles, label) {
  ...
  for (const file of siteFiles) {
    ...
    const publishedPath = relativePath === "index.html" ? "/" : `/${relativePath.replace(/index\.html$/, "")}`;
    if (!sitemapUrls.has(`${host}${publishedPath}`)) errors.push(`${file}: published URL is missing from ${label}`);
  }
  if (sitemapUrls.size !== siteFiles.length) {
    errors.push(`Expected ${siteFiles.length} unique URLs in ${label}, received ${sitemapUrls.size}`);
  }
}
