...
const distRoot = path.join(astroRoot, "dist", "zh");
const legacyRoot = path.resolve(astroRoot, "..", "zh");
const origin = "https://mnp-navi.jp";
const localeOrigin = `${origin}/zh`;
...
export async function validateChineseBuild() {
  const errors = [];
  ...
  if (htmlFiles.length !== 6834) errors.push(`expected 6,834 Chinese HTML pages, found ${htmlFiles.length}`);
  ...
  if (!sitemapEntries.every((url) => url.startsWith(`${localeOrigin}/`))) errors.push("sitemap contains a URL outside /zh/");
  ...
  for (const file of htmlFiles) {
    ...
    if (!/<html lang="zh-CN">/.test(html)) errors.push(`${label}: html lang is not zh-CN`);
    if (!/<meta property="og:locale" content="zh_CN">/.test(html)) errors.push(`${label}: og:locale is not zh_CN`);
    if (/rm-referral(?:-vi|-en|-zh|-ko|-pt)?\.maffun\.workers\.dev/.test(html)) errors.push(`${label}: legacy Worker URL remains`);
    if (!/href="\/zh\/css\/style\.css"/.test(html) || !/src="\/zh\/js\/analytics\.js"/.test(html)) errors.push(`${label}: Chinese static assets are missing`);
    ...
    if (/·\s*(?:ドコモ|ソフトバンク|イオンモバイル)\s+用户/.test(uiHtml)) errors.push(`${label}: Japanese carrier remains in eyebrow`);
    if (/<dt>当前通信公司<\/dt><dd>(?:ドコモ|ソフトバンク|イオンモバイル)<\/dd>/.test(uiHtml)) errors.push(`${label}: Japanese carrier remains in details`);
    ...
    for (const schema of schemas) {
      try {
        ...
        if (parsed["@type"] === "WebPage" && (parsed.url !== canonical || parsed.inLanguage !== "zh-CN")) errors.push(`${label}: WebPage schema mismatch`);
    ...
    for (const target of [...html.matchAll(/<(?:a|link|script|img)\b[^>]*(?:href|src)="([^"]+)"/gi)].map((match) => match[1].replaceAll("&amp;", "&")).filter((target) => target.startsWith("/"))) {
      ...
      if (!target.startsWith("/zh/")) { errors.push(`${label}: unprefixed internal target ${target}`); continue; }
...
const result = await validateChineseBuild();
