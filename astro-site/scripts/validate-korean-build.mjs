...
const distRoot = path.join(astroRoot, "dist", "ko");
const legacyRoot = path.resolve(astroRoot, "..", "ko");
const origin = "https://mnp-navi.jp";
const localeOrigin = `${origin}/ko`;
...
function routeToOutput(target) {
  ...
  if (!pathname.startsWith("/ko/")) return null;
...
export async function validateKoreanBuild() {
  const errors = [];
  ...
  if (htmlFiles.length !== 6834) errors.push(`expected 6,834 Korean HTML pages, found ${htmlFiles.length}`);
  ...
  if (!sitemapEntries.every((url) => url.startsWith(`${localeOrigin}/`))) errors.push("sitemap contains a URL outside /ko/");
  ...
  for (const file of htmlFiles) {
    ...
    if (!/<html lang="ko">/.test(html)) errors.push(`${label}: html lang is not ko`);
    if (!/<meta property="og:locale" content="ko_KR">/.test(html)) errors.push(`${label}: og:locale is not ko_KR`);
    if (/rm-referral(?:-vi|-en|-zh|-ko|-pt)?\.maffun\.workers\.dev/.test(html)) errors.push(`${label}: legacy Worker URL remains`);
    if (!/href="\/ko\/css\/style\.css"/.test(html) || !/src="\/ko\/js\/analytics\.js"/.test(html)) errors.push(`${label}: Korean static assets are missing`);
    ...
    const uiHtml = html
      ...
      .replace(/<ul class="coverage-shop-list"[\s\S]*?<\/ul>/gi, "")
      .replace(/출처: 라쿠텐 모바일 공식[^<]+/g, "");
    for (const phrase of forbiddenJapaneseUi) if (uiHtml.includes(phrase)) errors.push(`${label}: Japanese UI remains (${phrase})`);
    if (/·\s*(?:ドコモ|ソフトバンク|イオンモバイル)\s+이용자/.test(uiHtml)) errors.push(`${label}: Japanese carrier remains in eyebrow`);
    if (/<dt>현재 통신사<\/dt><dd>(?:ドコモ|ソフトバンク|イオンモバイル)<\/dd>/.test(uiHtml)) errors.push(`${label}: Japanese carrier remains in details`);
    ...
    for (const schema of schemas) {
      try {
        ...
        if (parsed["@type"] === "WebPage" && (parsed.url !== canonical || parsed.inLanguage !== "ko-KR")) errors.push(`${label}: WebPage schema mismatch`);
    ...
    for (const target of [...html.matchAll(/<(?:a|link|script|img)\b[^>]*(?:href|src)="([^"]+)"/gi)].map((match) => match[1].replaceAll("&amp;", "&")).filter((target) => target.startsWith("/"))) {
      ...
      if (!target.startsWith("/ko/")) { errors.push(`${label}: unprefixed internal target ${target}`); continue; }
...
const result = await validateKoreanBuild();
