...
test("all migrated shop pages pass SEO, link and sitemap validation", async () => {
  ...
  assert.equal(result.summary.shopPages, 3969);
  assert.equal(result.summary.generatedShopFiles, 3969);
  assert.equal(result.summary.uniqueCanonicals, 3969);
  assert.equal(result.summary.sitemapCoveredShopPages, 3969);
